...
| **TASK-LINK-001** | [LinkID] Валидация работы автогенерации DeepSeek v4 Pro и вебхуков | **LinkID Pro Post** | Чат LinkID (`@linkid_ai_bot`) | **Чат LinkID** | 🟡 P2 | 2026-09-12 | 📋 To Do |
...
- **Суть:** Оптимизация BGT: бот автоматически отключает или снижает позицию, когда ATR (волатильность) сжимается до исторических минимумов (флэт-болото), и резко активируется при пробое полос Боллинджера/ATR expansion. Это убережет от "распила" депозита в боковике.
- **Приоритет:** 🔴 P1 | **Ответственный:** Триада (BGT) | **Статус:** 📋 Backlog
...
| :--- | :--- | :--- | :--- | :---: |
| **2026-09-11** | **Alpha Stream (Идея №1)** | **Prediction Markets Arbitrage & Whale Scanner (Polymarket)** | Разработка модуля `alpha-scout` для поиска спредов/арбитража вероятностей 5m BTC и трекинга кошельков топ-50 трейдеров Polymarket с алертами в Telegram. | 📋 На обсуждение |
| **2026-09-11** | **BGT Upgrade** | **BGT Dynamic Volatility Regime (ATR Expansion Filter)** | Фильтр флэтового болота для BGT: вход в сделки только на расширении волатильности для исключения ложных входов. | 📋 На обсуждение |
...
### [TASK-LINKID-001] Восстановление работы генератора (402 Payment Required)
- **Суть:** В текущий момент генерация постов отваливается с ошибкой `HTTP Error 402: Payment Required` от OpenRouter. Это блокирует генерацию контента. Необходимо пополнить баланс API-ключа в OpenRouter либо перенастроить скрипт на альтернативного бесплатного/оплаченного провайдера (Gemini / OpenAI).
- **Приоритет:** 🔴 P0 (Blocker) | **Ответственный:** Владелец / LinkID Agent | **Статус:** 📋 To Do
- **DoD:** Генерация постов снова проходит успешно (возвращает 201), баланс/ключ обновлён.
