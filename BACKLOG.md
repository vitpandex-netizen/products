...
| **TASK-BGT-058** | **Спринт 4** | **HyperArb** — межбиржевой funding-арбитраж Bitget↔Hyperliquid (perp-perp, обходит лимит UTA на шорт спота). Скринер с фильтром цена/ликвидность/устойчивость-48ч + `funding_arb_cross.py` (fail-closed leverage, откат при незакрытой второй ноге, сверка факт.qty после открытия). Пилот $14 live с 19.09 (XMR, дельта-нейтрально подтверждено). ⚠️ См. `TASK-ALPHA-003` — похожее по описанию, другой домен (Alpha Scout, не BGT), пометка Done 18.09 — не проверено, не дублируем без сверки на Консилиуме. | **BGT (Bitget Bot)** | Чат BGT | **Claude Code** | 🔴 P1 | — | 🟡 In Progress (pilot live) |
