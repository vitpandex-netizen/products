...
**Дубли-имена без bgt-, ДОРМАНТНЫЕ (не крашатся, но стоит убрать при случае — низкий приоритет):**
`bot-watchdog` (vs `bgt-watchdog`), `daily-digest` (vs `bgt-digest`), `bitget-bot` (generic), `funding-arb-guard`
